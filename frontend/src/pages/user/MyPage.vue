    ...
    <main class="max-w-2xl mx-auto px-4 py-8">
      ...
      <template v-else>
        ...
        <section class="mb-8">
          ...
          <div v-if="data.approved_participations.length === 0" class="text-sm py-4 text-center glass-card p-4" :style="`color: var(--text-faint);`">
            参加予定の募集はありません
          </div>
        ...
        <section class="mb-8">
          ...
          <div v-if="data.pending_participations.length === 0" class="text-sm py-4 text-center glass-card p-4" :style="`color: var(--text-faint);`">
            申請中の募集はありません
          </div>
          <div v-else class="flex flex-col gap-3">
            <div
        ...
        <section>
          ...
          <div v-if="data.posted_recruitments.length === 0" class="text-sm py-4 text-center glass-card p-4" :style="`color: var(--text-faint);`">
            投稿した募集はありません
          </div>
          <div v-else class="flex flex-col gap-3">
            <div
            ...
            >
              <div class="flex items-center justify-between gap-3">
                ...
                <div class="text-right flex-shrink-0">
                  <span
                    class="text-xs font-bold px-2.5 py-0.5 rounded-full inline-block"
                    :class="r.status === 'open'
