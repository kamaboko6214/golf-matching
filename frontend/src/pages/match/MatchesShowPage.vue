        ...
        <div class="glass-card overflow-hidden mb-4">
          <div v-if="recruitment.image_url" class="relative h-52">
            ...
            <div class="absolute inset-0"
              style="background: linear-gradient(to top, rgba(4,10,5,0.9) 0%, transparent 55%);"></div>
            <div class="absolute bottom-0 left-0 right-0 p-5">
              <div class="flex items-center gap-2 mb-1.5">
                ...
                <span class="ml-auto text-xs font-bold px-2.5 py-0.5 rounded-full" :class="recruitment.status === 'open'
              ...
              <h1 class="text-2xl font-black text-white"
                style="font-family: 'Bebas Neue', sans-serif; letter-spacing: 0.05em;">{{ recruitment.title }}</h1>
            </div>
          </div>
          ...
          <div class="p-6">
            <template v-if="!recruitment.image_url">
              <div class="flex items-center gap-2 mb-3">
                <span class="badge-green">{{ recruitment.prefecture }}</span>
                <span class="text-xs" :style="`color: var(--text-faint);`">{{ formatDate(recruitment.play_date)
                  }}</span>
              ...
              <h1 class="text-2xl font-black mb-3 c-text"
                style="font-family: 'Bebas Neue', sans-serif; letter-spacing: 0.05em;">{{ recruitment.title }}</h1>
            </template>

            <p class="text-sm leading-relaxed mb-5" :style="`color: var(--text-muted);`">{{ recruitment.description }}
            </p>
            ...
            <div class="grid grid-cols-2 gap-3 py-4" :style="`border-top: 1px solid var(--border-card);`">
              ...
              <div class="flex flex-col gap-1">
                ...
                <span class="font-black c-text-accent"
                  style="font-family: 'Bebas Neue', sans-serif; font-size: 1.3rem; letter-spacing: 0.05em;">
                  {{ recruitment.needed_players }}<span class="text-sm font-normal ml-1 c-text-muted"
                    style="font-family: inherit;">名</span>
                </span>
              </div>
              <div class="flex flex-col gap-1">
        ...
        <div class="glass-card p-6 mb-4">
          <div v-if="recruitment.is_owner" class="text-center">
            ...
            <div class="flex flex-col gap-2">
              <button @click="$router.push(`/matches/${recruitment.id}/edit`)"
              ...
              <button @click="delete_recruitment()"
              ...
              <button v-if="approvedParticipation" @click="$router.push(`/chats/${approvedParticipation.chat_id}`)"
          ...
          <button v-else-if="!recruitment.my_participation && recruitment.status === 'open'" @click="applyParticipation"
            :disabled="applying" class="btn-primary w-full">
          ...
          <div v-else-if="recruitment.my_participation === 'pending'" class="text-center py-2">
            <span
              class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-bold bg-yellow-100 text-yellow-800 border border-yellow-200 dark:bg-yellow-900/30 dark:text-yellow-400 dark:border-yellow-700/30">
              ⏳ 申請中 — 承認をお待ちください
            </span>
          </div>
          ...
          <div v-else-if="recruitment.my_participation === 'approved'" class="text-center">
            ...
            <button @click="$router.push(`/chats/${recruitment.my_chat_id}`)" class="btn-primary w-full">
              💬 チャットを開始する
            </button>
          </div>
          ...
          <div v-else-if="recruitment.my_participation === 'rejected'" class="text-center py-2">
            <span
              class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-bold bg-red-100 text-red-700 border border-red-200 dark:bg-red-950/40 dark:text-red-400 dark:border-red-800/30">
              ❌ 申請が却下されました
            </span>
          </div>
          ...
          <div v-if="errorMsg"
            class="mt-3 text-sm px-4 py-3 rounded-lg bg-red-100 text-red-700 border border-red-200 dark:bg-red-950/30 dark:text-red-400 dark:border-red-800/30">
            {{ errorMsg }}
          </div>
        </div>

        <!-- 申請者一覧（投稿者のみ） -->
        <div v-if="recruitment.is_owner && recruitment.participations?.length > 0" class="glass-card p-6">
          <h2 class="section-title mb-4">参加申請者一覧</h2>
          <div class="flex flex-col gap-3">
            <div v-for="p in recruitment.participations" :key="p.id"
              class="flex items-center justify-between py-3 last:border-0"
              :style="`border-bottom: 1px solid var(--border-card);`">
              <div>
                <span @click.stop="$router.push(`/users/${p.user.id}`)"
                ...
                <span class="text-xs font-bold px-2.5 py-0.5 rounded-full mt-1 inline-block" :class="{
                ...
                <span
                  class="text-xs font-bold right-0 ml-4 px-2 py-0.5 rounded-full bg-gray-100 text-gray-500 border border-gray-200 dark:bg-gray-700/30 dark:text-gray-400 dark:border-gray-600/30">
                  平均スコア: {{ p.user.average_score ?? '未設定' }}
                </span>
              </div>
              <div v-if="p.status === 'pending'" class="flex gap-2">
                <button @click="updateParticipation(p.id, 'approved')"
                  class="bg-lime-400 text-green-950 text-xs font-black px-3 py-1.5 rounded-lg hover:brightness-110 transition">
                  承認
                </button>
                <button @click="updateParticipation(p.id, 'rejected')"
...
const  updateParticipation = async(participationId, status) => {
  try {
...
const delete_recruitment = async() => {
